...
function openMyhotelEditDialog($trigger) {
	...
	setValueImage("myhotel-item-image", $pathLiveTV
...
function checkErrorMyhotelItemCreate(name, link, image) {
	if (checkRequiredField(name)) {
		showMessageCheck("myhotel-item-container-error",
				getValueField("message-hotel-name"));
		return false;
	}
	if (checkRequiredField(link)) {
		showMessageCheck("myhotel-item-container-error",
				getValueField("message-hotel-link"));
		return false;
	}
				...
				if (file.size >= $fileSize) {
					...
					if (mode == 1) {
						...
						if (width < 135 || height < 200) {
							image.src = "";
							fileName.value = "";
							showMessageCheck(idDiv,
									"Please choose image with width &#8805; 135px, height &#8805; 200px");
							destroyCropImage("myhotel-item-image");
						} else {
							closeMessageCheck(idDiv);
						...
						if (file.size < $fileSizeMin) {
							...
							fileName.value = "";
							showMessageCheck(idDiv,
									getValueField("message-image-size-min"));
							destroyCropImage("myhotel-item-image");
						} else {
							$('#myhotel-div-item-image').addClass(
									'div-item-image');
							destroyCropImage("myhotel-item-image");
