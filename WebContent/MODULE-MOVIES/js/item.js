...
function createTableItem(data) {
	...
	$.each(data, function(i, item) {
		...
		var director = unescape(item.director);
		var productor = unescape(item.productor);
		var actor = unescape(item.actor);
		...
		col4.appendChild(createItemImageMovie($pathVideo + image));
		
		var col5 = row.insertCell(4);
		col5.appendChild(createItemInvisible_(id,name,director,productor,actor,plot,link,image,invisible,price,iunit,isnew));
		
		var col6 = row.insertCell(5);
		col6.innerHTML = price + " " + iunit;
		
		var col7 = row.insertCell(6);
		var btnEdit = createButton(1);
		var str = "'" + id + "','" + name + "', '" + director + "', '" + productor + "', '" + actor + "', '" 
			+ plot + "', '" + link + "', '" + image + "', '" + invisible + "', '" + price + "', '" + iunit + "', '" + isnew + "'";
...
function createItemInvisible_(id,name,director,productor,actor,plot,link,image,invisible,price,iunit,isnew) {
	var str = "'" + id + "','" + name+ "','" + director+ "','" + productor+"','"+actor+"','"+plot+"','"+link+"','"+image+"','"+invisible+"','"+price+"','"+iunit+"','"+isnew+"'";
...
function changeStatusData(id,name,director,productor,actor,plot,link,image,invisible,price,iunit,isnew){
	...
	$.ajax({
		...
		success : function(response) {
			...
			if (response > 0) {
				getItem();
				//showMessageSuccess(getValueField("movies-edit-success"));
			} else {
				showMessageError(getValueField("movies-edit-fail"));
			}
		}
	});
	}
...
function openNewDialog() {
	...
	$("#item-title").html(getValueField("movies-create-title"));
	setValueField("item-subject", $subjectId);
	setValueField("item-name", "");
	setValueField("item-director", "");
	setValueField("item-productor", "");
	setValueField("item-actor", "");
	setValueField("item-description", "");
	setValueField("item-link", "");
	setValueField("item-price", "");
	setValueField("item-iunit", "VND");
...
function openEditDialog(id, name, director, productor, actor, plot, link, image, invisible, price, iunit, isnew) {
	openDialog("item-dialog");
	destroyCropImage("item-image");
	$(".process-upload").hide();
	$("#container-ftp").hide();
	$("#media-player").show();
	closeMessageCheck("item-container-error");
	$("#item-title").html(getValueField("movies-edit-title"));
	setValueField("item-subject", $subjectId);
	setValueField("item-name", name);
	setValueField("item-director", director);
	setValueField("item-productor", productor);
	setValueField("item-actor", actor);
	setValueField("item-description", plot);
	setValueField("item-link", link);
	setValueField("item-price", price);
	setValueField("item-iunit", iunit);
	setValueField("item-file", "");
	setValueField("item-file-name", "");
...
function createItem(fileName, mediaName, result) {
	$.ajax({
		...
		data : {
			...
			productor :getValueField("item-productor"),
			director : getValueField("item-director"),
			actor :getValueField("item-actor"),
			poster : fileName,
			plot : getValueField("item-description"),
			price : getValueField("item-price"),
			iunit : getValueField("item-iunit"),
			url : mediaName,
		...
		success : function(response) {
			...
			if (response > 0) {
				...
				//showMessageSuccess(getValueField("movies-create-success"));
			} else {
				showMessageError(getValueField("movies-create-fail"));
			}
		}
	});
}
...
function deleteItem() {
	$.ajax({
		...
		success : function(response) {
			...
			if (response > 0) {
				getItem();
				//showMessageSuccess(getValueField("movies-delete-success"));
			} else if (response == -2) {
				showMessageError(getValueField("movies-delete-duplicate"));
			} else {
				showMessageError(getValueField("movies-delete-fail"));
			}
		}
	});
}
...
function checkErrorItemCreate(name, link, image) {
	if (checkRequiredField(name)) {
		showMessageCheck("item-container-error", getValueField("message-movie-name"));
		return false;
	} 
	if (checkRequiredField(link)) {
		showMessageCheck("item-container-error", getValueField("message-movie-link"));
		return false;
	} 
