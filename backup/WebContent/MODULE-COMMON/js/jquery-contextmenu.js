...
(function($, undefined) {
	...
	defaults = {
		...
		position : function(opt, x, y) {
			...
			var bottom = $win.scrollTop() + $win.height(), right = $win
					.scrollLeft()
					+ $win.width(), height = opt.$menu.height(), width = opt.$menu
					.width();
	...
	handle = {
		...
		mousedown : function(e) {
			...
			if ($currentTrigger && $currentTrigger.length
				...
				$currentTrigger.data('contextMenu').$menu
		...
		key : function(e) {
			...
			case 38: // up
				...
				if (opt.isInput) {
					if (e.keyCode == 9 && e.shiftKey) {
						...
						opt.$selected
								&& opt.$selected
										.find('input, textarea, select').blur();
						opt.$menu.trigger('prevcommand');
						return;
					} else if (e.keyCode == 38
							&& opt.$selected.find('input, textarea, select')
									.prop('type') == 'checkbox') {
			...
			case 36: // home
				if (opt.$selected
					...
					(opt.$selected && opt.$selected.parent() || opt.$menu)
							.children(':not(.disabled, .not-selectable)')[e.keyCode == 36 ? 'first'
							: 'last']().trigger('contextmenu:focus');
					e.preventDefault();
					return;
				}
				break;
			...
			default: // 0-9, a-z
				...
				if (opt.accesskeys[k]) {
					...
					opt.accesskeys[k].$node
							.trigger(opt.accesskeys[k].$menu ? 'contextmenu:focus'
									: 'mouseup');
					return;
				}
				break;
		...
		prevItem : function(e) {
			...
			var $children = opt.$menu.children(), $prev = !opt.$selected
					|| !opt.$selected.prev().length ? $children.last()
					: opt.$selected.prev(), $round = $prev;
		...
		nextItem : function(e) {
			...
			var $children = opt.$menu.children(), $next = !opt.$selected
					|| !opt.$selected.next().length ? $children.first()
					: opt.$selected.next(), $round = $next;
		...
		focusInput : function(e) {
			var $this = $(this).closest('.context-menu-item'), data = $this
					.data(), opt = data.contextMenu, root = data.contextMenuRoot;
		...
		itemMouseenter : function(e) {
			...
			(opt.$menu ? opt : root).$menu.children('.hover').trigger(
					'contextmenu:blur');
		...
		itemClick : function(e) {
			...
			if (!opt.items[key]
					|| $this
							.is('.disabled, .context-menu-submenu, .context-menu-separator, .not-selectable')) {
				return;
			}
		...
		focusItem : function(e) {
			...
			$this.addClass('hover').siblings('.hover').trigger(
					'contextmenu:blur');
	...
	op = {
		...
		hide : function(opt, force) {
			...
			$trigger.removeData('contextMenu').removeClass(
					"context-menu-active");
		...
		create : function(opt, root) {
			...
			// create contextMenu
			opt.$menu = $('<ul class="context-menu-list"></ul>').addClass(
					opt.className || "").data({
				'contextMenu' : opt,
					...
					.each(
							...
							function(key, item) {
								var $t = $(
										...
										.addClass(item.className || ""), $label = null, $input = null;
								...
								if (typeof item == "string") {
									$t
											.addClass('context-menu-separator not-selectable');
									...
									if (item.type == 'html') {
										$t
												.addClass('context-menu-html not-selectable');
									} else if (item.type) {
										$label = $('<label></label>').appendTo(
												$t);
										$('<span></span>').html(
												item._name || item.name)
												.appendTo($label);
		...
		update : function(opt, root) {
			...
			opt.$menu
					...
					.each(
							function() {
								var $item = $(this), key = $item
										.data('contextMenuKey'), item = opt.items[key], disabled = ($
										.isFunction(item.disabled) && item.disabled
										.call($trigger, key, root))
										|| item.disabled === true;
								...
								if (item.type) {
									...
									$item.find('input, select, textarea').prop(
											'disabled', disabled);
									...
									case 'checkbox':
									case 'radio':
										item.$input.val(item.value || "").prop(
		...
		layer : function(opt, zIndex) {
			...
			var $layer = opt.$layer = $(
					...
					.css({
						...
						width : $win.width(),
						display : 'block'
					}).data('contextMenuRoot', opt).insertBefore(this).on(
							'contextmenu', handle.abortevent).on('mousedown',
							handle.layerClick);
	...
	$.contextMenu = function(operation, options) {
		...
		case 'create':
			...
			if (!initialized) {
				...
				$document.on({
					...
					'mouseleave.contextMenu' : handle.menuMouseleave
						...
						'.context-menu-input', handle.inputClick).on({
			...
			switch (o.trigger) {
			case 'hover':
				$context.on('mouseenter' + o.ns, o.selector, o,
						handle.mouseenter).on('mouseleave' + o.ns, o.selector,
						o, handle.mouseleave);
				break;
			...
			if (_hasContext) {
				...
				$.each(menus, function(ns, o) {
					...
					if ($visibleMenu.length
							&& $visibleMenu.data().contextMenuRoot.$trigger
									.is($(o.context).find(o.selector))) {
	...
	$.contextMenu.setInputValues = function(opt, data) {
		...
		$.each(opt.inputs, function(key, item) {
			...
			case 'radio':
				item.selected = (data[item.radio] || "") == item.value ? true
						: false;
				break;
	...
	function menuChildren(items, $children, counter) {
		...
		$children.each(function() {
			var $node = $(this), node = this, nodeName = this.nodeName
					.toLowerCase(), label, item;
