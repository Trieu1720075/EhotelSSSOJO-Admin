	...
	$.ajax({
		...
		success : function(response) {				
		    ...
		    $.each(response, function (i, item) {
		        combo+="<option value='"+item.room+"'>" + item.room + "</option>";
		    });
		    select.append(combo);
...
function Load_Language(response){	
	...
	$.each(response, function(i, item) {		
		...
		iconLanguage.setAttribute('onclick', "openDialogEditLanguage('"+item.idLang+"','"+item.name+"','"+item.code+"','"+item.flagimage+"')");
...
function save_data_content(){ 
	...
	if(document.getElementById("file_logo").files.length == 0 && document.getElementById("file_logo_small").files.length == 0){		
		save_data_to_database(txt_gioithieu,txt_dichvu,txt_file_logo_old,txt_file_logo_small_old);
	}else if(document.getElementById("file_logo").files.length != 0 && document.getElementById("file_logo_small").files.length != 0){
		upload_two_logo();
		save_data_to_database(txt_gioithieu,txt_dichvu,txt_file_logo_old,txt_file_logo_small_old);
	...
	var name_logo_small= $("#file_logo_small_old").val();
	 ...
	 $.ajax({
			...
			success : function(response) {
				...
				save_data_to_database(txt_gioithieu,txt_dichvu,$name_images,name_logo_small);
				load_data_content();
			}
		});
	
}
function save_data_to_database(gioithieu_,dichvu_,file_logo_,file_logo_small_){	
	$.ajax({
		type : "POST",
		url : $pathWebService+"system",		
		cache : false,
...
function openDialogEditLanguage(idLanguage,namelanguage,codelanguage,path_image) {	
...
function editLanguage(){
	...
	if(document.getElementById("file_lag_update").files.length != 0){
		var oData = new FormData(document.forms.namedItem("id_form_update_language"));	
...
function Load_List_Background() {
	...
	$.ajax({
		...
		success : function(response) {
			var html="";
			$.each(response, function(i, item) {	
				var str = "'" + item.id+"','"+item.image + "'";
				var str_invisible = "'" + item.id+"','"+item.image + "','"+item.background + "','"+item.invisible + "'";
				html+='<div class="col-md-3" style="margin-bottom: 10px;">';
				html+='<div class="thumbnail text-center">';
				html+='<div style="font-size: 60px">';								
				html+='<img alt="" src="'+$path_image_background+item.image+'"';
				html+='style="height: 150px; width: 100%;">';
				html+='</div>';
				//html+='<div class="div_bottom" style="padding: 0px;">';		
				...
				if(xx != 0){				
					...
					html+='<input type="checkbox" class="div_checkbok" name="radio_main" checked="checked" onclick="setmain_background('+str+')"><span class="label label-primary">Set BG</span>';
					html+='</label>';					
				}else{			
					
					html+='<label class="div_bottom">';
					html+='<input type="checkbox" class="div_checkbok"  name="radio_main" onclick="setmain_background('+str+')"><span class="label label-primary">Set BG</span>';
					html+='</label>';
				...
				html+='<button type="button" class="btnDelete" onclick="deleted_background('+str+')"></button>';
				if(item.invisible==1){
					html+='<input id="isactive'+item.id+'" name="isactive"  type="checkbox" data-toggle="toggle"  data-size="mini" onchange="getChangestatus('+str_invisible+')">';
				}else if(item.invisible==0){
					html+='<input id="isactive'+item.id+'" name="isactive" checked="checked"  type="checkbox" data-toggle="toggle" data-size="mini" onchange="getChangestatus('+str_invisible+')">';
				}
				html+='</div>';				
				html+='</div>';
				html+='</div>';
...
function addbackground_standby(){
	...
	var oData = new FormData(document.forms.namedItem("id_form_add_background_standby"));	
	oData.append("image_name_flag", image_name_flag);	
	oData.append("file_background", file_background);
		$.ajax({
			type : "POST",
	...
	$.ajax({
		...
		success : function(response) {
			...
			$.each(response, function(i, item) {	
				...
				if(xx != 0){				
					html+='<label class="div_bottom">';
					html+='<input type="checkbox" class="div_checkbok" name="radio_main" checked="checked" onclick="setmain_background_music('+str+')"><span class="label label-primary">Set BG</span>';
					html+='</label>';					
				}else{			
					
					html+='<label class="div_bottom">';
					html+='<input type="checkbox" class="div_checkbok"  name="radio_main" onclick="setmain_background_music('+str+')"><span class="label label-primary">Set BG</span>';
					html+='</label>';
				...
				html+='<button type="button" class="btnDelete" onclick="deleted_background('+str+')"></button>';
				if(item.invisible==1){
					html+='<input id="isactive_music'+item.id+'" name="isactive"  type="checkbox" data-toggle="toggle"  data-size="mini" onchange="getChangestatus_standby('+str_invisible+')">';
				}else if(item.invisible==0){
					html+='<input id="isactive_music'+item.id+'" name="isactive" checked="checked"  type="checkbox" data-toggle="toggle" data-size="mini" onchange="getChangestatus_standby('+str_invisible+')">';
				}
				html+='</div>';				
				html+='</div>';
				html+='</div>';
...
function addbackground_music(){
	...
	var oData = new FormData(document.forms.namedItem("id_form_add_background_music"));	
	oData.append("image_name_flag", image_name_flag);	
	oData.append("file_background", file_background);
		$.ajax({
			type : "POST",
	...
	$.ajax({
		...
		success : function(response) {
			...
			$.each(response, function(i, item) {	
				...
				html+='<button type="button" class="btnDelete" onclick="deleted_background('+str+')"></button>';
				if(item.invisible==1){
					html+='<input id="isactive_music'+item.id+'" name="isactive"  type="checkbox" data-toggle="toggle"  data-size="mini" onchange="getChangestatus_music('+str_invisible+')">';
				}else if(item.invisible==0){
					html+='<input id="isactive_music'+item.id+'" name="isactive" checked="checked"  type="checkbox" data-toggle="toggle" data-size="mini" onchange="getChangestatus_music('+str_invisible+')">';
				}
				html+='</div>';				
				html+='</div>';
				html+='</div>';
